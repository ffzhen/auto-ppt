...
function getTemplate001Prompt({ isStream }: TemplatePrompt): string {
  console.log('getTemplate001Prompt')
...
function getTemplate003Prompt({ isStream }: TemplatePrompt): string {
  ...
  return `你是一个生成养生风格卡片内容的专家。请输出以下格式的完整JSON对象，每个对象代表一页卡片，不要遗漏原文要点，确保所有要点完整输出，特别是原文涉及要点数量时，保证要点数量和原文一致：
...
1. 封面页格式有2种，一种是主标题+副标题+背景图片，一种是html标题+背景图片，主副标题由完整标题拆分得到，例如："从春到夏减脂排毒黄金时间"拆分得到"从春到夏"和"减脂排毒黄金时间"例如：
{
  ...
  "data": {
    ...
    "background": {
      ...
      "workflow_id": "7507632455408713728"
    }
  }
}
...
1. 首先输出封面页：标题将会作为大字报的形式展示，标题为10-30字
{
  "type": "cover",
  "data": {
    "title": "主标题（10-30字），保证文字在页面中不溢出，自动添加eomji和重点文字高亮，高亮文字用<span class='highlight'>{text}</span>来表示",
  }
}
...
function getTemplate005Prompt({ isStream }: TemplatePrompt): string {
  console.log('getTemplate005Prompt')
  const streamPlaceholder = isStream 
    ? '每个对象输出后会立即处理，所以确保每个对象都独立有效' 
    : '所有对象应该组成一个有效的JSON数组'
...
2. 内容页有2种格式随机生成，items中至少3个要点,很重要！text生成的文案可以是html片段，自动添加eomji和html高亮元素，正文文字颜色为黑色，可加粗、画线或者加高亮样式，高亮样式（文字颜色#4F81BD并加粗）
内容1:{
  "type": "content",
  "data": {
    "title": "页面标题",
...
function getTemplate006Prompt({ isStream }: TemplatePrompt): string {
  console.log('getTemplate006Prompt')
  const streamPlaceholder = isStream 
    ? '每个对象输出后会立即处理，所以确保每个对象都独立有效' 
    : '所有对象应该组成一个有效的JSON数组'
...
1. 首先输出封面页：标题将会作为大字报的形式展示，标题为10-30字
{
  "type": "cover",
  "data": {
    "title": "主标题（10-30字），自动添加eomji和重点内容，自动高亮重点，高亮重点颜色为#FFDE5A或#4874CB，有人味，像和朋友分享的语气，吸引人点进去，例如："想找一个炒股厉害的男生带我回本"",
  }
}
...
// 复制 007 模板逻辑为 010 模板
function getTemplate010Prompt({ isStream }: TemplatePrompt): string {
...
1. 首先输出封面页：标题将会作为大字报的形式展示，标题为10-30字
{
  ...
  "data": {
    ...
    "text": "副标题分为三句，直击用户情绪，每句字数在7-9个汉字，三句话错落，逐渐变短",
  }
}
...
function getTemplate008Prompt({ isStream }: TemplatePrompt): string {
  console.log('getTemplate008Prompt')
  const streamPlaceholder = isStream 
    ? '每个对象输出后会立即处理，所以确保每个对象都独立有效' 
    : '所有对象应该组成一个有效的JSON数组'
