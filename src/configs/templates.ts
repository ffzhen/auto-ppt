...
// 生成现代简约风格模板的提示词
function getModernTemplatePrompt({ isStream }: TemplatePrompt): string {
  const streamPlaceholder = isStream 
    ? '每个对象输出后会立即处理，所以确保每个对象都独立有效' 
    : '所有对象应该组成一个有效的JSON数组'
...
export function getTemplatePrompt(templateId: string, isStream: boolean = false): string {
  switch (templateId) {
    ...
    case 'template006':
      return getModernTemplatePrompt({ isStream })
    default:
